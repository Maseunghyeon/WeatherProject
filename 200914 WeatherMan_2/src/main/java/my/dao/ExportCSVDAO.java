...
public class ExportCSVDAO {
	...
	public boolean exportCSV() {
		...
		// exportList 제대로 왔나 테스트 해보기
		for(UserMenuHitVO vo : exportList) {
			System.out.print("u_idx: "+vo.getU_idx()+"  ");
			System.out.print("m_idx: "+vo.getM_idx()+"  ");
			System.out.print("hit: "+vo.getHit());
			System.out.println();
		}*/
	...
	// 해당데이터의 csv엑스포트를 수행하는 메소드 // -- 이 부분 (static인메소드가 2개이면 거슬리므로, 이 부분 위의 메소드안에 포함시켜줄지, 아님 다른방법으로 해볼지 결정해서 해결하기)
	public static boolean exportUserMenuHit(List<UserMenuHitVO> exportList, String savePath, String fileName){
	    ...
	    try {
	        ...
	        if(!dir.isDirectory()){  // 해당주소에 디렉토리경로가 없을 경우   // File객체.isDirectory() : 파일객체가갖고있는주소가 실제로 디렉토리인지, 즉 해당 주소로 된 디렉토리경로가 지금 존재하고있는지 확인하는 boolean 메소드
	            dir.mkdirs();        // 해당주소로 디렉토리를 생성한다
	        }

	        bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(savePath + "/"+fileName+".csv"), "UTF-8"));

	        for(UserMenuHitVO vo : exportList) { // VO객체에 담아서 오는게아니라 Map에 임의로 담겨 오더라도, 칼럼명은 이미알고있으므로 순서대로 칼럼명의값을 가져와 String에담아주면 될듯하다
	        	// 고정배열(3칸짜리)을 만들기 : 아래 각 요소별 유효성검사할 때, vo객체형보다 반복문돌리기쉬운 타입인 배열(기왕이면고정배열)로 만들어 사용하기 위함
